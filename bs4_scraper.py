...
comm_data_old = pd.read_csv("comments_caa_facebook.csv", engine="python")
...
#reactions
for d in soup.find_all("div", attrs={'class':'dh'}):
	reactions = d.text
	print(reactions)
...
for d in soup.find_all("div"):
	if d.get('id') != None and d.get('id').isdigit() == True:
		...
		#comment
		try:
			comm_div = d.find('div', attrs={'class':'ee'})
			...
			print(comment)
		#likes
		like = d.find('a', attrs={'class':'ek cc'})
		try:
...
data_old.to_csv("caa_facebook.csv")
comm_data_old.to_csv("comments_caa_facebook.csv")
